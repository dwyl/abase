...
var joiAssert = function (object, schema) {
  return function () { Joi.assert(object, schema) };
}
...
test('schemaCreator returns simplejoi which behaves as expected', function (t) {
  ...
  t.throws(
    joiAssert({dob: '12-21-2012', username: 'ttt'}, schema),
...
test('schemaCreator with custom options', function (t) {
  var schema = schemaCreator(exampleSchema, [
    { name: 'email', options: { required: true } },
    { name: 'dob', options: { valid: ['12-21-2012', '12-22-2012'] } },
    'username'
  ]);
  ...
  t.throws(
    joiAssert({ email: 't@g.com', dob: '12-25-2012' }, schema),
    'specified dobs not given, example of user of array of arguments'
    + ' and additional requirements'
  );
  t.throws(
    joiAssert({ email: 't@g.com', username: 'ts'  }, schema),
