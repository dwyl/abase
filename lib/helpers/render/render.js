...
handlebars.registerPartial('string', fs.readFileSync(path.join(__dirname, 'partials/string.html'), 'utf8'));
handlebars.registerPartial('integer', fs.readFileSync(path.join(__dirname, 'partials/integer.html'), 'utf8'));
handlebars.registerPartial('boolean', fs.readFileSync(path.join(__dirname, 'partials/boolean.html'), 'utf8'));
handlebars.registerPartial('hidden', fs.readFileSync(path.join(__dirname, 'partials/hidden.html'), 'utf8'));
...
module.exports = function(page, request) {
  if (page.type === 'login') {
    ...
    return template({path: page.path, method: 'POST', fieldsToDisplay: page.fields, userFields: userFields});
  }
  if (page.type === 'signup') {
  ...
  if (page.type === 'user_edit') {
    ...
    var userFields = require(path.join(__dirname, '../../fields.json'));
    return template({userDetails: userData, fieldsToDisplay: page.fields, userFields: userFields });
  }
}